...
<p class="portfolio-subtitle">Portfolio 2: Logistics and Fulfillment Optimization | Zhiyi Zhu (Elsie Chu)</p>
...
Table 1 and Figures 1–3 compare the baseline nearest-center policy with the final transportation allocation model. The baseline policy had a lower total transportation cost and a shorter weighted average distance. However, it caused 4 capacity violations and 26,201 units of excess demand. The optimized model increased total transportation cost by 17.11%, but it reduced both capacity violations and excess demand to 0. This shows that a lower-cost and shorter-distance policy may not be feasible when fulfillment center capacity is considered.
...
Figure 4 shows the utilization rates of the 12 candidate fulfillment centers after optimization. No fulfillment center had a utilization rate above 1.00. The busiest center reached a utilization rate of 1.00, which means it was fully used but did not exceed its estimated capacity. This result shows that the optimized model satisfied the capacity constraints.
...
Table 2 and Figures 5–7 show how the optimization results changed under different capacity buffer values. As the capacity buffer increased from 1.00 to 1.30, total transportation cost, weighted average distance, and average utilization rate all decreased. More available capacity gave the model more flexibility to assign demand to closer fulfillment centers. The maximum utilization rate remained at 1.00 because at least one lower-cost or closer fulfillment center was fully used in each scenario, but no center exceeded its capacity.
